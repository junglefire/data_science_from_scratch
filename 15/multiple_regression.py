...
def bootstrap_sample(data):
    ...
    after = [random.choice(data) for _ in data]
    return after
...
def p_value(beta_hat_j, sigma_hat_j):
    if beta_hat_j > 0:
        return 2 * (1 - pb.normal_cdf(beta_hat_j / sigma_hat_j))
    else:
        return 2 * pb.normal_cdf(beta_hat_j / sigma_hat_j)
...
def estimate_beta_ridge(x, y, alpha):
    ...
    with penalty alpha"""
    beta_initial = [random.random() for x_i in x[0]]
    return gd.minimize_stochastic(partial(squared_error_ridge, alpha=alpha),
...
if __name__ == "__main__":
    ...
    # 第一组中位数的standard_deviation接近0
    # 第二组中位数的standard_deviation接近100
    print("bootstrap_statistic(close_to_100, median, 100):")
    print(bootstrap_statistic(close_to_100, st.median, 100))
    print("bootstrap_statistic(far_from_100, median, 100):")
    print(bootstrap_statistic(far_from_100, st.median, 100))
    print()
    """
    ...
    bootstrap_standard_errors = [
        st.standard_deviation([beta[i] for beta in bootstrap_betas])
        for i in range(4)]
    ...
    for alpha in [0.0, 0.01, 0.1, 1, 10]:
        ...
        print("alpha", alpha)
        print("beta", beta)
        print("dot(beta[1:],beta[1:])", la.dot(beta[1:], beta[1:]))
        print("r-squared", multiple_r_squared(x, daily_minutes_good, beta))
        print()
