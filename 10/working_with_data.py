...
def group_by(picker, rows, value_transform=None):
    # key is output of picker, value is list of rows
    grouped = defaultdict(list)
    for row in rows:
        grouped[picker(row)].append(row)
...
if __name__ == "__main__":
    ...
    X = [[2,3], [-2, 1], [3,-1], [4,1]]
    # 原始图
    # plt.scatter(get_column(X, 0), get_column(X, 1))
    # plt.title("Image 1")
    # plt.show()
    ...
    Y = de_mean_matrix(X)
    print("Y=", Y)
    # plt.scatter(get_column(Y, 0), get_column(Y, 1))
    # plt.title("Image 2")
    # plt.show()
